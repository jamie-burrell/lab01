...
NA stands for Not Available and this is because there are attractions for which there is no n_2021 value in the data set.
...
The mean statistics are skewed by a few attractions which received a huge number of visitors eg Kew Gardens, Royal Parks Windsor. The vast majority of attractions are more local and received a much smaller number of visitors meaning that the median remains lower.
...
visitors_with_nations %>%
  count(nation)
```
Northern Ireland's IQR seems overly high for its sole 9 attractions while Scotland's seems particularly low given its comparatively high number of 97 attractions.
